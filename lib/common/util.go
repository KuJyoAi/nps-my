...
//	}
func Getverifyval(vkey string, RandomKey string) string {
	b, err := crypt.AesEncrypt([]byte(vkey), []byte(RandomKey[0:16]))
	if err != nil {
		// fmt.Printf("err: %v\n", err)
		log.Fatalln(err)
	}
	// fmt.Printf("RandomKey: %v\n", RandomKey)
	// fmt.Printf("AES加密后的验证vkey %v\n", string(b))
	return string(b)
	// return crypt.Md5(vkey)
}
...
/*
 */
func GetRandom() (err error, str string) {
	i, err1 := rand.Int(rand.Reader, big.NewInt(2312321748173871283))
	if err1 != nil {
		return err1, "1"
	}
	return nil, i.String()
