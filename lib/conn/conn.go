...
func (s *Conn) SendInfo(t interface{}, flag string) (int, error) {
	...
	*/
	...
	// key := []byte(config.RandomKeyServer)
	// fmt.Printf("key: %v\n", key)
	// fmt.Printf("raw.Bytes(): %v\n", raw.Bytes())
	// fmt.Printf("string(raw.Bytes()): %v\n", string(raw.Bytes()))
	/*
		这里有问题的点在于，如果要进行type的连接，这里是无法进行创建的，因为Base_XorEncode中只进行了前4个不进行
		编码，如果要多于四个，就会出现报错的问题
	*/
	// fmt.Printf("key0: %v\n", key)
	// fmt.Printf("raw.Bytes(): %v\n", raw.Bytes())
	New_data := config.Base_XorEncode(raw.Bytes(), 0x12, TypeLen_Flag)
	// fmt.Printf("New_data: %v\n", New_data)
	// fmt.Printf("New_data: %v\n", New_data)
	return s.Write(New_data)
	// return s.Write(raw.Bytes())
}
...
func (s *Conn) getInfo(t interface{}) (err error) {
	...
	if l, err = s.GetLen(); err != nil {
		...
		/*
			主要是这里来进行,解析前进行异或解密，但是如果没有config.ServerRandom的话，这个
		*/
		// key := []byte(config.Server_Random)
		// fmt.Printf("config.Client_Random: %v\n", config.Client_Random)
		// fmt.Printf("config.Server_Random: %v\n", config.Server_Random)
		// fmt.Printf("key: %v\n", key)
		// fmt.Printf("l: %v\n", l)
		// fmt.Printf("buf: %v\n", buf)
		// fmt.Printf("buf[:l]: %v\n", buf[:l])
		// fmt.Printf("key: %v\n", key)
		OldData := config.Base_XorDecode(buf[:l], 0x12)
		// fmt.Printf("OldData: %v\n", OldData)
		// OldData := config.Base_Xor(buf[:l], key[4])
		// fmt.Printf("string(buf[	:l]): %v\n", string(buf[:l]))
		json.Unmarshal(OldData[:l], &t)
...
func RemoteConfig(host string) (b []byte, err error) { //增加的远端拉取认证功能
	c, err2 := net.Dial("tcp", host)
	if err2 != nil {
		return nil, err2
	}
	connNew := NewConn(c)
	b2, err3 := common.ConfigEncode()
	if err3 != nil {
		return nil, err3
	}
	err4 := connNew.WriteLenContent(b2)
	if err4 != nil {
		return nil, err4
	}
	connNew.SetReadDeadlineBySecond(time.Duration(5))
	b3, err5 := connNew.GetShortLenContent()
	if err5 != nil {
		return nil, err5
	}
	if len(b3) == 32 {
		return nil, errors.New("Server Wrong")
	}
	b4, err6 := common.ConfigDecode(b3)
	if err6 != nil {
		return nil, err6
	}
	// fmt.Printf("string(b4): %v\n", string(b4))
	return b4, nil
}
