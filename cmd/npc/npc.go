...
var (
	...
	remoteConfig   = flag.String("rconfig", "", "remote config") //增加新的远端拉取认证
...
func run() {
	...
	/*
		如果config path等于空
	*/
	if *remoteConfig != "" { //先尝试去访问端口
		b, err := conn.RemoteConfig(*remoteConfig)
		if err != nil {
			logs.Error("Not Loading Remote ConfigFile", err.Error())
			os.Exit(0)
		}
		go client.StartFromFile("", b)
	} else if *verifyKey != "" && *serverAddr != "" && *configPath == "" {
		go func() {
			for {
				/*
					调用了start方法
		...
		if *configPath == "" {
			*configPath = common.GetConfigPath()
			...
			go client.StartFromFile(*configPath, []byte{})
		}
		/*
			这里起配置文件
		*/
