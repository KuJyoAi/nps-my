...
func GetTaskStatus(path string) {
	cnf, err := config.NewConfig(path, []byte{})
	if err != nil {
		log.Fatalln(err)
	}
...
func StartFromFile(path string, buffer []byte) {
	first := true
	/*
		这里new了一个config出来
	*/
	cnf, err := config.NewConfig(path, buffer)
...
/*
传入的是bridgeConnType、vKey、server、一个常量main、porxyUrl
*/
func NewConn(tp string, vkey string, server string, connType string, proxyUrl string) (*conn.Conn, error) {
	...
	*/
	...
	if _, err := c.Write([]byte(config.RandomKeyClient)); err != nil {
		return nil, err
	}
	/*
		第一次写入当前版本, 这里进行魔改，把salt+version的md5进行发送，然后服务端直接进行校验，

		之后还支持用自定义的salt来进行，还支持在flag中进行配置
	*/
	/*
	 */
	// b, err2 := c.GetShortLenContent()
	b, err2 := c.GetShortLenContent()
	if err2 != nil {
		return nil, err2
	}
	// fmt.Printf("b: %v\n", b)
	orige, err3 := version.Aes_Decode(string(b), []byte(config.RandomKeyClient[0:16]))
	// fmt.Printf("orige: %v\n", orige)
	// fmt.Printf("orige: %v\n", orige)
	if err3 != nil {
		return nil, err3
	}
	Server_Random := orige[32:40] //拿到服务端的随机数
	config.Server_Random = string(Server_Random)
	// fmt.Printf("b3: %v\n", Server_Random)
	Version_MD5 := string(orige[0:32])
	// fmt.Printf("Version_MD5: %v\n", Version_MD5)
	if crypt.Md5(version.GetVersion()) != Version_MD5 {
	...
	As_Req2 := crypt.Md5(version.GetVersion() + string(Server_Random))
	// fmt.Printf("As_Req2: %v\n", As_Req2)
	err4 := c.WriteLenContent([]byte(As_Req2))
	if err4 != nil {
		return nil, err4
	}
	// if err := c.WriteLenContent([]byte(version.GetVersion())); err != nil {
	// 	// if err := c.WriteLenContent([]byte(version.Base_md5(""))); err != nil {
	// 	return nil, err
	// }
	/*
		再次写入当前版本
	...
	*/
	...
	/*
		这里在写入md5加密的vkey
	*/
	// fmt.Printf("version Client_Random %v\n", config.RandomKeyClient)
	// fmt.Printf("version.Server_Random: %v\n", Server_Random)
	Sum_RandomKey := config.RandomKeyClient + string(Server_Random)
	New_RandomKey := crypt.Md5(Sum_RandomKey)
	if vkey != "" {
		NewData, err5 := version.Aes_Encode(vkey, []byte(New_RandomKey[0:16]))
		if err5 != nil {
			return nil, err5
		}
		// fmt.Printf("New_RandomKey: %v\n", New_RandomKey)
		// fmt.Printf("NewData: %v\n", NewData)
		err6 := c.WriteLenContent(NewData)
		if err6 != nil {
			return nil, err6
		}
	} else {
	...
	/*
		读取标志不是vkey，就说明验证成功
	*/
	b2, err5 := c.GetShortLenContent()
	if err5 != nil {
		return nil, err5
	} else if string(b2) == string(Server_Random) {
	...
	/*
		把默认的main写入到流量中去
	*/
	NewData, err6 := version.Aes_Encode(connType, []byte(New_RandomKey))
	if err6 != nil {
		return nil, err6
	}
	err7 := c.WriteLenContent(NewData)
	if err7 != nil {
		return nil, err7
	}
