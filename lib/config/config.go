...
var (
	RandomKeyClient = crypt.GetRandomString(32) //客户端生成的random_key
	RandomKeyServer = crypt.GetRandomString(8)  //服务端生成的random_key
	Server_Random   string                      //客户端收到的-客户端使用
	Client_Random   string                      //服务端收到的-服务端使用
)
...
var (
	_, Auth_key = common.GetRandom()
)

func NewConfig(path string, buffer1 []byte) (c *Config, err error) {
	c = new(Config)
	var b []byte
	/*
		读了config中的文件，然后进行了解析
	*/
	if len(buffer1) != 0 {
		b = buffer1
	} else {
...
flag 是用来判断参数是否进行编码
*/
func Base_XorEncode(source []byte, key byte, flag bool) (New []byte) {
	// fmt.Printf("source: %v\n", source)
	var TypeLen int
	if flag {
		TypeLen = 8
	...
	for i := 0; i < len(source); i++ {
		if i < TypeLen {
			// fmt.Printf("source[i]: %v\n", source[i])
			New = append(New, source[i])
			continue
		} else {
			New = append(New, source[i]^key)
		}
	}
	// fmt.Printf("New: %v\n", New)
	return
}
func Base_XorDecode(source []byte, key byte) (New []byte) {
	// fmt.Printf("source: %v\n", source)
	// var TypeLen int
	// if flag {
	// 	TypeLen = 8
