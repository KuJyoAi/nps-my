...
func (s *Bridge) cliProcess(c *conn.Conn) {
	//read test flag
	/*
		其实这里就是获取TST三个字段,这里魔改为32位的md5
	*/
	// if _, err := c.GetShortContent(3); err != nil {
	Client_RandomKey, err1 := c.GetShortContent(32)
	if err1 != nil {
		logs.Info("The client %s connect error", c.Conn.RemoteAddr(), err1.Error())
		return
	}
	//version check
	/*
		下面进行魔改第二部分通信,version的md5+随机数并用aes进行加密
	*/
	orige := crypt.Md5(version.GetVersion()) + config.RandomKeyServer
	// fmt.Printf("[]byte(version.Server_Random): %v\n", []byte(config.RandomKeyServer))
	NewData, err2 := version.Aes_Encode(orige, Client_RandomKey[0:16])
	if err2 != nil {
		logs.Info("The client %s connect error", c.Conn.RemoteAddr(), err2.Error())
	}
	err4 := c.WriteLenContent(NewData)
	if err4 != nil {
		return
	}
	// c.SetReadDeadlineBySecond(5)
	// fmt.Println("11")
	b, err3 := c.GetShortLenContent() //这里拿到的就是一个（服务器随机数+version）的MD5
	if err3 != nil {
		// fmt.Printf("err3: %v\n", err3)
		c.Close()
		return
	...
	// fmt.Printf("version.GetVersion(): %v\n", version.GetVersion())
	// fmt.Printf("version.Server_Random: %v\n", version.Server_Random)
	// fmt.Printf("crypt.Md5(version.GetVersion() + version.Server_Random): %v\n", crypt.Md5(version.GetVersion()+version.Server_Random))
	if string(b) != crypt.Md5(version.GetVersion()+config.RandomKeyServer) {
	...
	*/
	...
	/*
		从client中获取vkey的值,这里魔改为了buf1的aes加密后的值
	*/
	buf1, err5 := c.GetShortLenContent()
	// fmt.Printf("buf1: %v\n", buf1)
	...
	config.Client_Random = string(Client_RandomKey)
	// fmt.Printf("string(Client_RandomKey): %v\n", string(Client_RandomKey))
	// fmt.Printf("string(version.Server_Random): %v\n", string(config.RandomKeyServer))
	New_RandomKey := crypt.Md5(string(Client_RandomKey) + config.RandomKeyServer)
	// fmt.Printf("New_RandomKey: %v\n", New_RandomKey)
	...
	//verify
	// 验证vkey是否正确
	id, err := file.GetDb().GetIdByVerifyKey(string(buf1), c.Conn.RemoteAddr().String(), New_RandomKey)
	/*
		如果连接失败就发送服务端的md5，如果成功就发送客户端的md5
	*/
	if err != nil {
		/*
		 */
		...
		err6 := c.WriteLenContent([]byte(config.RandomKeyServer))
		if err6 != nil {
			return
		}
		// s.verifyError(c)
		...
		/*
		 */
		err6 := c.WriteLenContent(Client_RandomKey)
		if err6 != nil {
			return
		}
		// s.verifySuccess(c)
	}
	flag, err6 := c.GetShortLenContent()
	if err6 != nil {
		logs.Warn(err, flag)
	} else {
		b2, err7 := crypt.AesDecrypt(flag, []byte(New_RandomKey))
		if err7 != nil {
			return
		}
		// fmt.Println("11111")
		// fmt.Printf("string(b2): %v\n", string(b2))
		s.typeDeal(string(b2), c, id, string(vs))
	}
